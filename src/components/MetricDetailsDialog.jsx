import { StaggerContainer, ScaleIn } from './MotionWrappers';
...
export function MetricDetailsDialog({ title, dataList, children }) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[90vh] overflow-hidden flex flex-col sm:max-w-3xl rounded-3xl">
        <DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto py-2 custom-scrollbar pr-2">
          <StaggerContainer delay={0.03}>
            <div className="grid gap-3">
              {dataList.length > 0 ? dataList.map((item, i) => (
                <ScaleIn key={i}>
                  <div className="flex flex-col flex-wrap sm:flex-row sm:items-center justify-between gap-2 rounded-2xl border border-slate-100 bg-slate-50 p-4 transition-colors hover:bg-slate-100">
                    <div>
